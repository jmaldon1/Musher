...
if(NOT MUSHER_PYTHON_VERSION)
  set(MUSHER_PYTHON_VERSION "" CACHE STRING "Python version to use for compiling modules")
endif()

set(Python_ADDITIONAL_VERSIONS 3.8 3.7 3.6 3.5 3.4)
find_package(PythonLibsNew ${MUSHER_PYTHON_VERSION} REQUIRED)

include(CheckCXXCompilerFlag)
include(CMakeParseArguments)

# Add a CMake parameter for setting cpp standard
if(NOT MUSHER_CPP_STANDARD AND NOT CMAKE_CXX_STANDARD)
  if(NOT MSVC)
    check_cxx_compiler_flag("-std=c++17" HAS_CPP17_FLAG)

    if (HAS_CPP17_FLAG)
      set(MUSHER_CPP_STANDARD -std=c++17)
    else()
    ...
    #   set(MUSHER_CPP_STANDARD -std=c++14)
    # else()
  ...
  set(MUSHER_CPP_STANDARD ${MUSHER_CPP_STANDARD} CACHE STRING
      "C++ standard flag, e.g. -std=c++17, -std=c++20, /std:c++17.  Defaults to C++17 mode." FORCE)
endif()
...
function(_musher_add_lto_flags target_name prefer_thin_lto)
  ...
  if (MUSHER_LTO_CXX_FLAGS)
    target_compile_options(${target_name} PRIVATE "$<$<NOT:$<CONFIG:Debug>>:${MUSHER_LTO_CXX_FLAGS}>")
  endif()
  if (MUSHER_LTO_LINKER_FLAGS)
    target_link_libraries(${target_name} PRIVATE "$<$<NOT:$<CONFIG:Debug>>:${MUSHER_LTO_LINKER_FLAGS}>")
  endif()
endfunction()
...
function(musher_add_module target_name)
  ...
  target_include_directories(${target_name} ${inc_isystem}
    PRIVATE ${MUSHER_INCLUDE_DIR}  # from project CMakeLists.txt
